...
public class ExtendedGenerator {
    ...
    private int TDKI_SUBCLASSES_MAX = 20;
    private int TDKI_PERCENTAGE;
    /**
     ...
     * data file writer
     */
    private ExtendedWriter writer_;
    /**
    ...
    private int tdki_dept_subclass;
    private String tdki_output_dir;
    ...
    public static void main(String[] args) {
        //default values
        int univNum = 1, startIndex = 0, seed = 0, hole = 5, subclass = 20;
        boolean daml = false;
        ...
        try {
            ...
            while (i < args.length) {
                ...
                } else if (arg.equals("-hole")) {
                    if (i < args.length) {
                        arg = args[i++];
                        hole = Integer.parseInt(arg);
                        if (hole < 0) {
                            throw new NumberFormatException();
                        }
                    }
                } else if (arg.equals("-subclass")) {
                    if (i < args.length) {
                        arg = args[i++];
                        subclass = Integer.parseInt(arg);
                        if (subclass < 0) {
                            throw new NumberFormatException();
                        }
                    }
            ...
            System.err.println("Usage: ExtendedGenerator\n"
                    ...
                    + "\t[-daml]\n"
                    + "\t[-hole <incompleteness_percentage(0~100)>]\n"
                    + "\t[-subclass <no_of_subject_subclasses(1~" + Integer.MAX_VALUE + ")>]\n"
                    + "\t[-dir <output_directory>]\n"
                    + "\t-onto <univ-bench ontology url>");
            System.exit(0);
        }

        new ExtendedGenerator().start(univNum, startIndex, seed, daml, ontology, hole, subclass, dir);
    }
    ...
    public void start(int univNum, int startIndex, int seed, boolean daml,
            String ontology, int hole, int subclass, String outputDirectory) {
        this.ontology = ontology;
        this.tdki_output_dir = outputDirectory;
        
        isDaml_ = daml;
        if (daml) {
            writer_ = new ExtendedDamlWriter(this);
        } else {
            writer_ = new ExtendedOwlWriter(this);
        }

        startIndex_ = startIndex;
        baseSeed_ = seed;
        TDKI_PERCENTAGE = hole;
        TDKI_SUBCLASSES_MAX = subclass;
    ...
    private void _generate() {
        ...
        try {
            File tempDir = new File(tdki_output_dir);
            File logFile = new File(tempDir, LOG_FILE);
            ...
            System.out.println("Failed to access the output directory or create the log file!");
        }
    }
    ...
    private void tdki_makeDept(int deptIndex) {
        tdki_dept_subclass = _getRandomFromRange(1, TDKI_SUBCLASSES_MAX);
        writer_.tdki_addConceptAssertion("Subj" + tdki_dept_subclass + "Department", _getId(CS_C_DEPT, deptIndex));
    }

    private void tdki_makeProf(int profType, int profIndex) {
        writer_.tdki_addConceptAssertion("Subj" + tdki_dept_subclass + "Professor", _getId(profType, profIndex));
    }

    private void tdki_makeStudent(int studentType, int studentIndex) {
        writer_.tdki_addConceptAssertion("Subj" + tdki_dept_subclass + "Student", _getId(studentType, studentIndex));
    }

    private void tdki_makeCourse(int courseType, int courseIndex) {
        writer_.tdki_addConceptAssertion("Subj" + tdki_dept_subclass + "Course", _getId(courseType, courseIndex));
    }

    private boolean tdki_chooseToAdd() {
        boolean result = true;
        int n = _getRandomFromRange(1, 100);
        if (n <= TDKI_PERCENTAGE) {
            result = false;
        }
        return result;
    }
     ...
     * instances[CS_C_UNIV].count till generateASection(CS_C_UNIV, ) is invoked.
     */
    private void _generateDept(int univIndex, int index) {
        String fileName = new File(tdki_output_dir, _getName(CS_C_UNIV, univIndex) + INDEX_DELIMITER + index + _getFileSuffix()).getAbsolutePath();
    ...
    private void _generateASection(int classType, int index) {
        ...
        switch (classType) {
            ...
            case CS_C_UNDERSTUD:
                ...
                tdki_makeStudent(classType, index);
                break;
            case CS_C_GRADSTUD:
    ...
    private void _assignGraduateStudentPublications(String author, int min, int max) {
        ...
        for (int i = 0; i < list.size(); i++) {
            publication = (PublicationInfo) publications_.get(((Integer) list.get(i)).intValue());
            publication.authors.add(author);
        }
    }
    ...
    private void _generateAnUndergraduateStudent(int index) {
        ...
        if (tdki_chooseToAdd()) {
            ...
            for (int i = 0; i < list.size(); i++) {
                CourseInfo info = (CourseInfo) underCourses_.get(((Integer) list.get(i)).intValue());
    ...
    private void _generateAGradudateStudent(int index) {
        ...
        if (tdki_chooseToAdd()) {
            ...
            for (int i = 0; i < list.size(); i++) {
                CourseInfo info = (CourseInfo) gradCourses_.get(((Integer) list.get(i)).intValue());
    ...
    private void _generateACourse(int index) {
        ...
        tdki_makeCourse(CS_C_COURSE, index);
    }
    ...
    private void _generateAGraduateCourse(int index) {
        ...
        tdki_makeCourse(CS_C_GRADCOURSE, index);
    }
    ...
    private void _generateRaTa() {
        ...
        for (i = 0; i < instances_[CS_C_TA].total; i++) {
            ...
            ta.indexInCourse = ((CourseInfo) underCourses_.get(((Integer) courseList.get(i)).intValue())).globalIndex;
            _generateATa(ta);
        }
        while (i < list.size()) {
            ra = new RaInfo();
